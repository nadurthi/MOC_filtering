function pdfnorm = get_interp_pdf_0I_boostmixGaussian_2D(X,probs,mquad,Pquad,Nm,Ngcomp,Tk,Xmctest,Xtruth,plotsconf)
%%
...
[~,bind] = min(costs);
lamGM = LAMS(:,bind);
lamGM(1) = lamGM(1)+log(factconst);
lamsolGM = lamGM;

mxentpoly_normGM=zeros(1,dim+1);
for i=1:length(lamsolGM)
    mxentpoly_normGM=add_sub_polyND(mxentpoly_normGM, scalar_multiply_polyND(lamsolGM(i),Pf{i}),'add');
end
mxentpoly_normGM=simplify_polyND(mxentpoly_normGM);

pdfnorm.funcGM =@(x)GaussSumMix(x,Ngcomp,wg,comps)+exp(evaluate_polyND(mxentpoly_normGM,x))+ mndiff - cm;
