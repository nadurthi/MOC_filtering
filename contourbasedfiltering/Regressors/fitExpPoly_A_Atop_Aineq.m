...
Xextra=zeros(Nextra*size(X,1),size(X,2));
bextra = zeros(Nextra*size(X,1),1);
for i=1:size(X,1)
    Xextra((i-1)*Nextra+1:i*Nextra,:) = mvnrnd(X(i,:),0.05^2*eye(dim),Nextra);
    %         bextra(i) = beq(i);
end
